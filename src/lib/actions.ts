...
export async function processUserTurn(input: ProcessUserTurnInput): Promise<ProcessUserTurnOutput> {
  ...
  // Images will no longer be generated by the AI flows based on the updated prompts and schemas.
  // let aiImageUrls: string[] | undefined = undefined; 

  try {
    if (language === 'Amharic') {
      // Use contextualChatReply for Amharic
      const amharicReply = await contextualChatReply({
        chatHistory: combinedAiHistory.map(m => ({...m, role: m.role === 'ai' ? 'assistant' : m.role } as any)), 
        userMessage: userMessageText, 
        language: 'Amharic',
      });
      aiReplyText = amharicReply.reply;
      // amharicReply.image_url will no longer exist
    } else {
      // Use generateImageIntegratedReply for English
      const englishReply = await generateImageIntegratedReply({
        chatHistory: combinedAiHistory.map(m => ({...m, role: m.role === 'assistant' ? 'ai' : m.role } as any)), 
        userMessage: userMessageText, 
      });
      aiReplyText = englishReply.reply;
      // englishReply.imageUrls will no longer exist
    }
  } catch (error) {
  ...
  const aiResponseMessage: ClientChatMessage = {
    ...
    role: 'ai',
    content: aiReplyText,
    // imageUrls: aiImageUrls, // This will be undefined as AI won't generate images
    timestamp: Date.now(),
  };
